...
while read line
    do
        ...
        printf " /woldlab/loxcyc/proj/genome/programs/hotspot2-2.1.1/scripts/hotspot2.sh -f 0.05 -F 0.05 -c "$hotspot_chrom_sizes" -C "$hotspot_center_sites" "$line"."$2"."$3".unique.dup.nochrM.bam "$line"."$2"."$3"Out/ && " >> testcodehotspot2
        printf "unstarch "$line"."$2"."$3"Out/"$(echo $line | rev | cut -d/ -f1 | rev)"."$2"."$3".unique.dup.nochrM.hotspots.fdr0.1.starch > "$line"."$2"."$3"Out/"$(echo $line | rev | cut -d/ -f1 | rev)"."$2"."$3".unique.dup.nochrM.hotspots.fdr0.1.bed && " >> testcodehotspot2
        printf "unstarch "$line"."$2"."$3"Out/"$(echo $line | rev | cut -d/ -f1 | rev)"."$2"."$3".unique.dup.nochrM.hotspots.fdr0.05.starch > "$line"."$2"."$3"Out/"$(echo $line | rev | cut -d/ -f1 | rev)"."$2"."$3".unique.dup.nochrM.hotspots.fdr0.05.bed && " >> testcodehotspot2
