...
while read line
    do
        ...
        printf " /woldlab/loxcyc/proj/genome/programs/hotspot2-2.1.1/scripts/hotspot2.sh -f 0.5 -F 0.5 -c "$hotspot_chrom_sizes" -C "$hotspot_center_sites" "$line"."$2"."$3".unique.dup.nochrM.bam "$line"."$2"."$3"Out/ && " >> testcodehotspot2
        ...
        printf " /woldlab/loxcyc/proj/genome/programs/hotspot2-2.1.1/scripts/hsmerge.sh -f 0.1 "$line"."$2"."$3"Out/"$(echo $line | rev | cut -d/ -f1 | rev)"."$2"."$3".unique.dup.nochrM.allcalls.starch "$line"."$2"."$3"Out/"$(echo $line | rev | cut -d/ -f1 | rev)"."$2"."$3".unique.dup.nochrM.hotspots.fdr0.1.starch && " >> testcodehotspot2
        printf "unstarch "$line"."$2"."$3"Out/"$(echo $line | rev | cut -d/ -f1 | rev)"."$2"."$3".unique.dup.nochrM.hotspots.fdr0.5.starch | grep -v chrM - | intersectBed -a - -b "$mitoblack" -v | intersectBed -a - -b "$blacklist" -v > "$line"."$2"."$3"Out/"$(echo $line | rev | cut -d/ -f1 | rev)"."$2"."$3".unique.dup.nochrM.hotspots.fdr0.5.bed && " >> testcodehotspot2
        ...
        printf "awk '{print \$1\"\\\t\"\$2\"\\\t\"\$3\"\\\t\"\$4\"\\\t\"\$5\"\\\t+\"}' "$line"."$2"."$3"Out/"$(echo $line | rev | cut -d/ -f1 | rev)"."$2"."$3".unique.dup.nochrM.hotspots.fdr0.5.bed > "$line"."$2"."$3"Out/"$(echo $line | rev | cut -d/ -f1 | rev)"."$2"."$3".unique.dup.nochrM.hotspots.fdr0.5.bed.temp && " >> testcodehotspot2 
        ...
        printf "/woldlab/loxcyc/proj/genome/programs/x86_64/bedToBigBed "$line"."$2"."$3"Out/"$(echo $line | rev | cut -d/ -f1 | rev)"."$2"."$3".unique.dup.nochrM.hotspots.fdr0.5.bed.temp "$chromsizes" "$line"."$2"."$3"Out/"$(echo $line | rev | cut -d/ -f1 | rev)"."$2"."$3".unique.dup.nochrM.hotspots.fdr0.5.bigBed && " >> testcodehotspot2
