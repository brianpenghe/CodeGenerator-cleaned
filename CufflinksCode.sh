...
while read line
    do
        printf "condor_run -a request_memory=20000 \" /woldlab/castor/proj/programs/cufflinks-2.2.1.Linux_x86_64/cufflinks -p 8 -q --no-update-check --max-bundle-length 4500000 --compatible-hits-norm --GTF /woldlab/castor/home/phe/genomes/mm9/Mus_musculus.NCBIM37.67.filtered.removingENSMUST00000127664.gtf --output-dir "$line"."$2"."$3"Cufflinks2.2.1 "$line"."$2"."$3"/accepted_hits.bam 2> "$line"."$2"."$3"Cufflinks.err \" && " >> testcodeCufflinks
        printf "condor_run -a request_memory=20000 \" sort -k 1d,1 "$line"."$2"."$3"Cufflinks2.2.1/isoforms.fpkm_tracking > "$line"."$2"."$3"Cufflinks2.2.1/isoforms.fpkm_trackingSorted \" && " >> testcodeCufflinks
