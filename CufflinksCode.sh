...
while read line
    do
        ...
        printf $(echo $line | rev | cut -d/ -f1 | rev)"\t"$line"."$2"."$3"Cufflinks2.2.1/genes.fpkm_trackingSorted\t0,1,5,7\t8\n" >> CufflinksgeneFPKMs 
        printf $(echo $line | rev | cut -d/ -f1 | rev)"\t"$line"."$2"."$3"Cufflinks2.2.1/genes.fpkm_trackingSorted\t0,1,5,7\t9\n" >> CufflinksgeneConflows
