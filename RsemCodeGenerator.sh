...
printf "paste <(head -1 FPKM.genes | awk '{print \$1\"\\\t\"\$2\"\\\t\"}') <(cat rsem.genes.results.list | paste -s) | cat - <(tail -n +2 FPKM.genes) | paste $GeneNames - > temp && mv temp rsem_FPKM_genes.tsv\n" >> testcode
printf "paste <(head -1 count.genes | awk '{print \$1\"\\\t\"\$2\"\\\t\"}') <(cat rsem.genes.results.list | paste -s) | cat - <(tail -n +2 count.genes) | paste $GeneNames - > temp && mv temp rsem_count_genes.tsv\n" >> testcode
printf "paste <(head -1 FPKM.isoforms | awk '{print \$1\"\\\t\"\$2\"\\\t\"}') <(cat rsem.isoforms.results.list | paste -s) | cat - <(tail -n +2 FPKM.isoforms) > temp && mv temp rsem_FPKM_isoforms.tsv\n" >> testcode
printf "paste <(head -1 count.isoforms | awk '{print \$1\"\\\t\"\$2\"\\\t\"}') <(cat rsem.isoforms.results.list | paste -s) | cat - <(tail -n +2 count.isoforms) > temp && mv temp rsem_count_isoforms.tsv\n" >> testcode
