...
while read line
    do
        ...
        printf "/woldlab/castor/proj/programs/x86_64/bedToBigBed "$line"."$2"."$3"lS50000mD50s150fL0.bed "$line"."$2"."$3"lS50000mD50s150fL0.bigBed & \n" >> testcode
    done <$1
...
printf '''
    for file in *.bigBed
        do
        echo "track type=bigBed name="$file" description="$file" maxHeightPixels=60:32:8 visibility=dense color=150,0,150 bigDataUrl=http://woldlab.caltech.edu/~phe/"$current_folder_name"/"$file >> tracksummary
        done
''' >> testcode
