...
while read line
    do
        ...
        printf ""$(echo $line | rev | cut -d/ -f1 | rev | sed -r s/[.-]/_/g)"lS50000mD50s150fL0.HOMERbackgroundscores=\$(bedtools intersect -u -a "$line"."$2"."$3".unique.dup.count.bg4 -b "$line"."$2"."$3"lS50000mD50s150fL0.HOMER.background.bed | awk '{if(\$4!=0){array1+=1;arrayX+=log(\$4)/log(2);arrayXsq+=(log(\$4)/log(2))^2}} END {print arrayX/array1\" \"sqrt(arrayXsq/array1 - arrayX^2/array1^2)}') && " >> testcodeHOMER
        printf ""$(echo $line | rev | cut -d/ -f1 | rev | sed -r s/[.-]/_/g)"lS50000mD50s150fL0.HOMERbackgroundmean=\$(echo \$"$(echo $line | rev | cut -d/ -f1 | rev | sed -r s/[.-]/_/g)"lS50000mD50s150fL0.HOMERbackgroundscores | cut -d ' ' -f1) && " >> testcodeHOMER
        printf ""$(echo $line | rev | cut -d/ -f1 | rev | sed -r s/[.-]/_/g)"lS50000mD50s150fL0.HOMERbackgroundstd=\$(echo \$"$(echo $line | rev | cut -d/ -f1 | rev | sed -r s/[.-]/_/g)"lS50000mD50s150fL0.HOMERbackgroundscores | cut -d ' ' -f2) && " >> testcodeHOMER
        printf "awk '{print \$1\"\\\t\"\$2\"\\\t\"\$3\"\\\t\"2^((log(\$4)/log(2)-'\$"$(echo $line | rev | cut -d/ -f1 | rev | sed -r s/[.-]/_/g)"lS50000mD50s150fL0.HOMERbackgroundmean')/'\$"$(echo $line | rev | cut -d/ -f1 | rev | sed -r s/[.-]/_/g)"lS50000mD50s150fL0.HOMERbackgroundstd')-1}' "$line"."$2"."$3".unique.dup.count.bg4 > "$line"."$2"."$3".unique.dup.2PowZscore.bg4 && " >> testcodeHOMER
