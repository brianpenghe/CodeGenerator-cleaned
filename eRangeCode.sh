...
while read line
    do
        printf "condor_run -a request_memory=30000 \"python /woldlab/castor/home/georgi/code/erange-4.0a/MakeRdsFromBam5.py reads "$line"."$2"."$3".unique.nochrM.bam "$line"."$2"."$3".unique.nochrM.rds --index --cache=20000000 && " >> testcodeeRange
        ...
        printf "sort -k 1d,1 -k 2n,3 "$line"."$2"."$3".unique.nochrM.3x.2RPM.bed | /usr/bin/intersectBed -a - -b "$mitoblack" -v | /usr/bin/intersectBed -a - -b "$blacklist" -v > "$line"."$2"."$3".unique.nochrM.3x.2RPM.clean.bed && mv "$line"."$2"."$3".unique.nochrM.3x.2RPM.clean.bed "$line"."$2"."$3".unique.nochrM.3x.2RPM.bed && " >> testcodeeRange
        printf "sort -k 1d,1 -k 2n,3 "$line"."$2"."$3".unique.nochrM.5x.4RPM.bed | /usr/bin/intersectBed -a - -b ~/genomes/mm9/mm9-blacklist.bed -v | /usr/bin/intersectBed -a - -b "$mitoblack" -v | /usr/bin/intersectBed -a - -b "$blacklist" -v > "$line"."$2"."$3".unique.nochrM.5x.4RPM.clean.bed && mv "$line"."$2"."$3".unique.nochrM.5x.4RPM.clean.bed "$line"."$2"."$3".unique.nochrM.5x.4RPM.bed && " >> testcodeeRange
