...
if [[ "$1" == "PE" ]]
    then
        ...
        printf "~/.local/bin/cutadapt -a CTGTCTCTTATACAC "$2"R1all.fastq > "$2"R1trimmedfastq 2> "$2"R1trimmedfastqcutadapt.log && " >> testcode
        printf "~/.local/bin/cutadapt -a CTGTCTCTTATACAC "$2"R2all.fastq > "$2"R2trimmedfastq 2> "$2"R2trimmedfastqcutadapt.log && " >> testcode
        printf "java -jar /woldlab/castor/proj/programs/Trimmomatic-0.33/trimmomatic-0.33.jar PE -threads 4 -trimlog "$2"trimmomatic.log "$2"R1trimmedfastq "$2"R2trimmedfastq "$2"R1allpairedfastq "$2"R1allunpairedfastq "$2"R2allpairedfastq "$2"R2allunpairedfastq ILLUMINACLIP:/woldlab/castor/home/phe/genomes/NexteraAdaptors.fa:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:"$(($3==0?20:$3))" && " >> testcode
    ...
    then
        ...
        printf "~/.local/bin/cutadapt -a CTGTCTCTTATACAC "$2"all.fastq > "$2"trimmedfastq 2> "$2"trimmedfastqcutadapt.log && " >> testcode
        printf "java -jar /woldlab/castor/proj/programs/Trimmomatic-0.33/trimmomatic-0.33.jar SE -threads 4 -trimlog "$2"trimmomatic.log "$2"trimmedfastq "$2"alltrimmedfastq ILLUMINACLIP:/woldlab/castor/home/phe/genomes/NexteraAdaptors.fa:2:30:10 MAXINFO:35:0.9 MINLEN:"$(($3==0?20:$3))" && " >> testcode
