...
if [[ "$1" == "PE" ]]
    then
        ...
        printf "java -jar /woldlab/castor/proj/programs/Trimmomatic-0.33/trimmomatic-0.33.jar PE -threads 4 -trimlog "$2"trim.log "$2"R1allfastq "$2"R2allfastq "$2"R1trimmedfastq "$2"R1allunpairedfastq "$2"R2trimmedfastq "$2"R2allunpairedfastq ILLUMINACLIP:NexteraAdaptors.fa:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:"$(($3==0?20:$3))" && " >> testcode
        printf "~/.local/bin/cutadapt -a CTGTCTCTTATACAC "$2"R1trimmedfastq > "$2"R1allpairedfastq 2> "$2"R1allpairedfastq.log && "
        printf "~/.local/bin/cutadapt -a CTGTCTCTTATACAC "$2"R2trimmedfastq > "$2"R2allpairedfastq 2> "$2"alltrimmedfastq.log && "
    ...
    then
        ...
        printf "java -jar /woldlab/castor/proj/programs/Trimmomatic-0.33/trimmomatic-0.33.jar SE -threads 4 -trimlog "$2"trim.log "$2"allfastq "$2"trimmedfastq ILLUMINACLIP:NexteraAdaptors.fa:2:30:10 MAXINFO:35:0.9 MINLEN:"$(($3==0?20:$3))" && " >> testcode
        printf "~/.local/bin/cutadapt -a CTGTCTCTTATACAC "$2"trimmedfastq > "$2"alltrimmedfastq 2> "$2"alltrimmedfastq.log && "
