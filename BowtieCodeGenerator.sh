...
while read path
    do
        printf "arguments=\"-c \' python /woldlab/castor/home/georgi/code/trimfastq.py "$path"allfastq "$3" -stdout | /woldlab/castor/proj/genome/programs/bowtie-1.0.1+hamrhein_nh_patch/bowtie "$bowtieindex" -p 8 -v 2 -k 1 -m 3 -t --sam-nh --best --strata -q --sam - | /woldlab/castor/proj/genome/programs/samtools-0.1.8/samtools view -bT  "$fa" - | /woldlab/castor/proj/programs/samtools-0.1.16/bin/samtools sort - "$path"."$2"."$3"mer.unique \' \"\nqueue\n" >> bowtie$bowtiedate".condor"
    done <$1
