...
while read path
    do
		printf "/woldlab/castor/proj/genome/programs/samtools-0.1.8/samtools sort -n "$path"."$2"."$3"mer/unmapped.bam "$path"."$2"."$3"mer/unmappedsorted.bam && " >> testcodeTophatunmapped
        ...
        printf "python /woldlab/castor/home/georgi/code/makewigglefromBAM-NH.py --- "$path"."$2"."$3"mer/unmapped.Patrick.bam ~/genomes/plasmids/bowtie2-index/PatrickCRISPR.chrom.sizes "$path"."$2"."$3"mer.unmapped.Patrick.bg4 -notitle -uniqueBAM -RPM & \n " >> testcodeTophatunmapped
    done <$1
